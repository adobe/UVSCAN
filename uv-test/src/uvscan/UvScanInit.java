 ...
 */
	public UvScanInit(String alfProductName , String alfCodeName , List<String> locales, String results_file)  
	{
		...
		log.info("RecievedParams for the AlfProductName:"+alfProductName+" , AlfCodeName:"+alfCodeName+" , locale:"+this.Locales);
	}
...
/**
 * Returns the RGMApiResultBean map for all locales for which UV-SCAN is initiated
 ...
 */
	public void runUvScan(String ldap, String csvpath)
	{
		...
		LocaleController en_US_LocaleController = new LocaleController(this.AlfProductName , this.AlfCodeName , "en_US" , generalController);

		for(int i=1 ;i<localesToBeProceessed.length; i++)
		{

			long startTime =  System.currentTimeMillis();
			log.info("Inside Server " + apiName + " for locale "+localesToBeProceessed[i]+" at " + startTime + " miliseconds");
			...
			//////////////UV-Scan analytics/////////////////////////////
			String logForAnalytics = "LDAP:"+userLdap+";SessionID:"+this.SessionID+";AlfProductName:"+this.AlfProductName+";AlfCodeName:"+this.AlfCodeName;	
			log.info("UV-Scan run by user - Final;"+(logForAnalytics + uvScanbean.toString()));
			

			long timeTaken = System.currentTimeMillis() - startTime;
			log.info("Total time by Server for " + apiName + " for locale "+localesToBeProceessed[i]+" is "+timeTaken+" miliseconds");
		...
		Map<String, UvScanResultBean> sortedMap = new TreeMap<String, UvScanResultBean>(generalController.LocaleMapBean);
		for(Map.Entry<String,UvScanResultBean> val :sortedMap.entrySet())
		{
			UvScanResultBean uvb= val.getValue();
			logForAnalytics = "LDAP:"+userLdap+";SessionID:"+this.SessionID+";AlfProductName:"+this.AlfProductName+";AlfCodeName:"+this.AlfCodeName;	
			log.info("UV-Scan run by user - Master;"+(logForAnalytics + uvb.toString()));
		}
		this.setUvScanResultFilePath( UvScanInit.Results_file+"\\"+Configuration.getConstant("UvScanResultsDirName")+"/"+Configuration.getConstant("UvScan_LocaleVsUrl_Result_File_FileName")+"."+Configuration.getConstant("UvScan_LocaleVsUrl_Result_File_FileExt")); 

		long CallEndTime =  System.currentTimeMillis();
		log.info("Total time taken by  "+ apiName+"  for all locale  is "+(CallEndTime-CallStartTime)+" miliseconds");
 ...
 */
	public static RGMApiResultBean getRGMApiResultBeanForUvscan(UvScanResultBean uvBean)
	{
		...
		try{
			...
			//result.setHttpStatus(HttpURLConnection.HTTP_INTERNAL_ERROR);
			result.setErrorTitle(Configuration.getConstant("UvScanResultMessageForExceptionScenario") + ex.getMessage());
			ex.printStackTrace();
...
/**
 * sets value of RGMApiResultBean when  UvScanResultBean is null or exception occurs
 * @return Map of RGMApiResultBean with locale as key
 */
